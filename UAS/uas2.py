...
def draw_sky_dome(tex_day, tex_night, day_factor):
    ...
    # Kita gambar langit malam dulu full opaque (atau transparan jika day_factor tinggi, tapi lebih aman full background)
    # Tapi agar transisi "pudar" kita pakai logika blend:
    # Blend: Night Sky di bawah, Day Sky di atas (Alpha = day_factor)
    
    # Draw Night Sky (Opaque Base)
    glBindTexture(GL_TEXTURE_2D, tex_night)
    glColor4f(1, 1, 1, 1.0) 
    glPushMatrix()
    glRotatef(90, 1, 0, 0) # Rotate agar texture tegak
    gluSphere(quad, 200, 32, 32)
    glPopMatrix()
    
    # 2. DAY SKY (Overlay dengan Alpha = day_factor)
    # Jika day_factor = 1 (Siang), ini menutupi malam sepenuhnya.
    # Jika day_factor = 0 (Malam), ini invisible.
    if day_factor > 0.01:
        glBindTexture(GL_TEXTURE_2D, tex_day)
        glColor4f(1, 1, 1, day_factor) # Alpha blending magic
        glPushMatrix()
        glRotatef(90, 1, 0, 0)
        # Sedikit lebih kecil agar tidak z-fighting jika depth test on, tapi kita depthmask false dan manual order.
        # Tetap sphere yang sama ok karena order render.
        gluSphere(quad, 199, 32, 32) 
        glPopMatrix()
